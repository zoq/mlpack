 ...
 */
#ifndef MLPACK_CORE_OPTIMIZERS_EVE_EVE_IMPL_HPP
#define MLPACK_CORE_OPTIMIZERS_EVE_EVE_IMPL_HPP
    ...
    arma::mat& iterate)
{
  ...
  for (size_t i = 0; i < actualMaxIterations; /* incrementing done manually */)
  {
    ...
    if ((currentFunction % numFunctions) == 0 && i > 0)
    {
      ...
      Log::Info << "EVE: iteration " << i << ", objective " << overallObjective
          << "." << std::endl;

      if (std::isnan(overallObjective) || std::isinf(overallObjective))
      {
        Log::Warn << "EVE: converged to " << overallObjective << "; terminating"
      ...
      {
        Log::Info << "EVE: minimized within tolerance " << tolerance << "; "
  ...
  Log::Info << "EVE: maximum iterations (" << maxIterations << ") reached; "
      << "terminating optimization." << std::endl;
